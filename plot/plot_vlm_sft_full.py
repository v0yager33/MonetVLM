#!/usr/bin/env python3
"""
绘制 VLM SFT 全参微调实验的 TensorBoard 日志图表
"""
import os
import sys
...
def plot_vlm_sft_full():
    """绘制 VLM SFT 全参微调实验的训练曲线"""
    
    log_dir = os.path.join(os.path.dirname(__file__), '..', 'save', 'vlm_sft_full', 'runs')
    ...
    if available_loss_tags:
        ...
        ax.set_title('VLM SFT (Full Model) - Training & Evaluation Loss', fontsize=14)
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3)
        
        output_path = os.path.join(output_dir, 'vlm_sft_full_loss.png')
    ...
    if lr_tags:
        ...
        ax.set_title('VLM SFT (Full Model) - Learning Rate', fontsize=14)
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3)
        
        output_path = os.path.join(output_dir, 'vlm_sft_full_lr.png')
        plt.savefig(output_path, dpi=150, bbox_inches='tight')
        print(f"已保存: {output_path}")
        plt.close()
    
    print("VLM SFT 全参微调实验绘图完成!")
