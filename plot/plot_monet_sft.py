#!/usr/bin/env python3
"""
绘制 MonetVLM SFT 实验的 TensorBoard 日志图表
"""
import os
import sys
...
def plot_monet_sft():
    """绘制 MonetVLM SFT 实验的训练曲线"""
    
    log_dir = os.path.join(os.path.dirname(__file__), '..', 'save', 'monet_sft', 'runs')
    ...
    if available_loss_tags:
        ...
        ax.set_title('MonetVLM SFT - Training & Evaluation Loss', fontsize=14)
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3)
        
        output_path = os.path.join(output_dir, 'monet_sft_loss.png')
    ...
    if lr_tags:
        ...
        ax.set_title('MonetVLM SFT - Learning Rate', fontsize=14)
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3)
        
        output_path = os.path.join(output_dir, 'monet_sft_lr.png')
        plt.savefig(output_path, dpi=150, bbox_inches='tight')
        print(f"已保存: {output_path}")
        plt.close()
    
    print("MonetVLM SFT 实验绘图完成!")
