...
- 用于训练 SparkVLM 的 约 110k 条数据，领域微调数据集约 22k 条数据
...
| **Phase 2-2.5** | `sft_train_full.py` | SFT 全参微调，解冻所有参数，领域微调复用该脚本 | 无 |
...
├── trainers/
│   ├── grpo_trainer.py          # 自定义 GRPO Trainer
...
- ShareGPT4V-coco（images，只下coco子集就行了)：https://modelscope.cn/datasets/thomas/ShareGPT4V
- ShareGPT4V-coco（text)：https://huggingface.co/datasets/AHAAM/ShareGPT4V-COCO
